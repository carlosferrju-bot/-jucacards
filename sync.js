(()=>{
  ...
  async function put(d){if(!valid(d)||syncing)return false;try{syncing=true;const r=await fetch('/api/data',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(d),keepalive:true,cache:'no-store'}),j=await r.json().catch(()=>null);if(r.status===409&&j?.data){const raw=JSON.stringify(j.data);nativeSet.call(localStorage,KEY,raw);nativeSet.call(localStorage,LAST,raw);location.reload();return false}if(!r.ok)throw new Error(j?.error||`sync ${r.status}`);const raw=JSON.stringify(d);nativeSet.call(localStorage,KEY,raw);nativeSet.call(localStorage,LAST,raw);return true}catch(e){console.warn('JucaCards: falha ao salvar no servidor',e);return false}finally{syncing=false}}
  async function syncBest(){if(syncing)return;const local=bestLocal();if(!local)return;try{const r=await fetch('/api/data',{cache:'no-store'});if(!r.ok){if(hasContent(local.data))await put(local.data);return}const j=await r.json(),remote=j?.data;if(!valid(remote)){if(hasContent(local.data))await put(local.data);return}const lc=cards(local.data),rc=cards(remote);if(rc===0&&lc>0){await put(local.data);return}if(rc>lc){const raw=JSON.stringify(remote);nativeSet.call(localStorage,KEY,raw);nativeSet.call(localStorage,LAST,raw);location.reload();return}if(lc>rc){await put(local.data)}}catch(e){console.warn('JucaCards: sincronização indisponível',e)}}
